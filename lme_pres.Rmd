...
A linear statistical model with both *fixed* and *random* effects. Non-formal definitions of fixed and random effects:

**Fixed effects** are the things you usually put in a regression model, such as gender, age, treatment, race, temperature, grade level, income, SAT score, etc, etc. We want to understand how they affect our dependent variable. _They produce regression coeffficients._

**Random effects** are things that affect fixed effects. Examples include individuals, teachers, schools, rats, plots of land, etc. We want to understand how their variation affects our fixed effects. _They produce estimates of variance, not regression coefficients._

## More formal definitions

Fixed effects are parameters associated with an entire population or with certain repeatable levels of experimental factors.
...
Google "fixed vs random effects". You'll find many different explanations. There is no consensus on how to define them.

On top of that linear mixed-effect models are also called "hierarchical linear models", "multilevel models", or "longitudinal models", with different terms and notation!

Mix in advanced statistics and a trying to learn a statistical software package, and you have something that can take some getting used to.

Today the goal is to get your started with basic linear mixed-effect modeling in the context of `R`.


## Candidate for linear mixed-effect modeling

We have a new medical treatment. We carry out an experiment where we randomly prescribe the treatment or placebo to patients and collect response data every two weeks for two months to gauge efficacy. 

- The Fixed Effect is Treatment. If we were to replicate this study, we would use this variable again.  

- The Random Effect is Patient. *Notice the response data is grouped by patient*. If we were to replicate this study, we would not use the same patients again.   
...
- The Fixed Effect is Curriculum. If we were to replicate this study, we would use this variable again. Other fixed effects might also include gender and race.  

- The Random Effects are Teachers and Schools. *Notice the response data is grouped by schools and teachers within schools*. If we were to replicate this study, we would not necessarily use the same teachers and schools again.


## Data for Linear Mixed-Effect Models

LME (**L**inear **M**ixed-**E**ffect) models are suitable for clustered, longitudinal or repeated-measures data in which the data are grouped by _random levels, out of a huge population of possible levels,_ and the dependent variable is continuous. 

In our first example, we had longitudinal data grouped by patient, with multiple observations on the same patient. The effect of the patient is a random effect. The effect of the treatment is a fixed effect.

In our second example, we had clustered data grouped by schools and teachers within schools. The effect of the schools and teachers within schools are random effects. The effect of the curriculum is a fixed effect.
...
In this matrix there are three parameters to estimate: $\sigma_{b1}^{2}$, $\sigma_{b2}^{2}$, and $\sigma_{b1,b2}$. This structure assumes the two random effects have covariance, ie, they vary together. *This is the default in R when you have two random effects.* 
...
There are other covariance structures for $\boldsymbol{R_i}$, but we won't cover those today.
