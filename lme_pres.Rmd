...
Fixed-effect factors with more than two levels should be assessed with `anova`, not by examining p-values in the `summary` output.
...
The `car` package provides a functions called `Anova` that provides approximate p-values and tests each predictor after all others as opposed to sequentially (aka, Type II). 
...
If you have a p-value close to significance, (say anout 0.10) you may want to consider calculating a corrected p-value using a *mixture of chi-square distributions*.
...
It turns out that $0.5\chi_{df}^{2} + 0.5\chi_{df-1}^{2}$ can serve as a reference null distribution for computing the p-value.
