...
**Fixed effects** are parameters for a population. These are regression coefficients for *repeatable* variables. (eg: Gender, Age, Treatment, Race, etc.) If we repeat the analysis on new data, the *repeatable* variables would be collected or used again.

**Random effects** are associated with individual units drawn at random from a population. (eg: subjects, teachers, schools, plots of land, etc.) We can think of these units as *non-repeatable* levels of factors. If we repeat the analysis on new data, we would not use the same individual units.
...
where $\boldsymbol{X\beta} = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + ... + \beta_k X_k$.
...
Notice the `i` subscript. Linear mixed-effect models are fit to groups of data, say repeated measures made on subjects. $\boldsymbol{X_i}$ are the predictors. $\boldsymbol{\beta}$ are the fixed effect coefficients. $\boldsymbol{Z_i}$ is a subset of the predictors. $\boldsymbol{b_i}$ are the random effect coefficients. The $\epsilon_i$ are the random errors.
...
In LME, the null reference distribution is technically not known, at least not for unbalanced data. Thus the `lme4` author elected to not output p-values based on a distribution that is not actually the distribution of the test statistic.
