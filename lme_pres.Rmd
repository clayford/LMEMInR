...
To asses normality of random effects, say a random intercept:

`qqnorm(ranef(lme1)[[1]]$"(Intercept)")`
...
We can compare models either by hypothesis testing or by selection criteria.
...
The function to compare models is `anova()`. It's default behavior is to refit the models with LM before comparing. 
...
When testing whether or not to include a random effect in a model, we're testing that its variance is 0. In that case our hypothesis test involves a values lying on the boundary of the parameter space.
