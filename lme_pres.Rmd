...
**Fixed effects** are parameters for a population. These are regression coefficients for *repeatable* variables. (eg: Gender, Age, Treatment, Race, etc.) If we repeat the analysis or experiment on new data, the *repeatable* variables would be collected or used again.

**Random effects** are associated with individual units drawn at random from a population. (eg: subjects, teachers, schools, plots of land, etc.) We might think of these units as *non-repeatable* levels of factors. If we repeat the analysis or experiment on new data, we would not use the same individual units.
...
LME (**L**inear **M**ixed-**E**ffect Models) are suitable for clustered, longitudinal or repeated-measures data in which the data are grouped by *random factors* and the response (or dependent variable) is continuous. 
...
## Fixed effects vs Random effects in LME

Fixed effects are _regression coefficients_. They describe the relationship between our response and predictors. They are assumed to be unknown fixed quantities that we try to estimate.

Random effects are _random values_ associated with the levels of a random factor. They usually represent random deviations from the relationships described by fixed effects. We're more interested in their variation than their actual values. 
...
## Fitting a LME for random slopes
...
## Fitting a LME for random intercepts and slopes
...
`lme4` also uses different computations than `nlme` that makes it better for larger data.
...
In LME, the null reference distribution is technically not known, at least not for unbalanced data. Thus the `lme4` authors elected to not output p-values based on a distribution that is not actually the distribution of the test statistic.
...
The `effects` package provides convenient functions for graphically displaying statistical models. 

Say you have a model object called `lme1`. Basic usage would be:

`library(effects)`   
`plot(allEffects(lme))`

This produces a plot of effects over "pretty" levels of your predictors.

The `effects` package is quite powerful. See `?allEffect` for many examples of visualizing several kinds of statistical models.

## Effects plot example

```{r, echo=FALSE}
library(effects)
plot(allEffects(lme3), main="Effect plot of model fit on slide 26")
```
